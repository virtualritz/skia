...
void ApplyArcToTangent(
        SkPath& p, SkScalar x1, SkScalar y1, SkScalar x2, SkScalar y2, SkScalar radius) {
...
void ApplyCubicTo(
        SkPath& p, SkScalar x1, SkScalar y1, SkScalar x2, SkScalar y2, SkScalar x3, SkScalar y3) {
    p.cubicTo(x1, y1, x2, y2, x3, y3);
}

void ApplyRCubicTo(SkPath& p,
                   SkScalar dx1,
                   SkScalar dy1,
                   SkScalar dx2,
                   SkScalar dy2,
...
void ApplyLineTo(SkPath& p, SkScalar x, SkScalar y) { p.lineTo(x, y); }

void ApplyRLineTo(SkPath& p, SkScalar dx, SkScalar dy) { p.rLineTo(dx, dy); }

void ApplyMoveTo(SkPath& p, SkScalar x, SkScalar y) { p.moveTo(x, y); }

void ApplyRMoveTo(SkPath& p, SkScalar dx, SkScalar dy) { p.rMoveTo(dx, dy); }
...
EMSCRIPTEN_BINDINGS(Skia) {
    ...
    class_<SkCanvas>("Canvas")
            ...
            .function("_concat", optional_override([](SkCanvas& self, WASMPointerF32 mPtr) {
                          ...
                          const SkScalar* sixteenMatrixValues =
                                  reinterpret_cast<const SkScalar*>(mPtr);
            ...
            .function("_drawCircle",
                      select_overload<void(SkScalar, SkScalar, SkScalar, const SkPaint& paint)>(
                              &SkCanvas::drawCircle))
            ...
            .function("_drawImageRectOptions",
                      ...
                      allow_raw_pointers())
            .function("_drawLine",
                      select_overload<void(SkScalar, SkScalar, SkScalar, SkScalar, const SkPaint&)>(
                              &SkCanvas::drawLine))
            .function(
                    "_drawOval",
                    optional_override(
            ...
            .function("_drawParagraph",
                      optional_override([](SkCanvas& self,
                                           skia::textlayout::Paragraph* p,
                                           SkScalar x,
                                           SkScalar y) { p->paint(&self, x, y); }),
                      allow_raw_pointers())
#endif
            ...
            .function("_drawTextBlob",
                      select_overload<void(
                              const sk_sp<SkTextBlob>&, SkScalar, SkScalar, const SkPaint&)>(
            ...
            .function("rotate",
                      select_overload<void(SkScalar, SkScalar, SkScalar)>(&SkCanvas::rotate))
    ...
    class_<SkFont>("Font")
            ...
            .function("_getGlyphIDs",
                      optional_override([](SkFont& self,
                          ...
                          int actualCodePoints = self.textToGlyphs(
                                  str, strLen, SkTextEncoding::kUTF8, glyphIDs, expectedCodePoints);
            ...
            .function("_getGlyphIntercepts",
                      optional_override([](SkFont& self,
                          ...
                          auto sects = self.getIntercepts(glyphs.data(),
                                                          SkToInt(glyphs.size()),
                                                          (const SkPoint*)pos.data(),
                                                          top,
                                                          bottom);
    ...
    class_<SkImageFilter>("ImageFilter")
            ...
            .class_function(
                    "MakeBlur",
                    optional_override([](SkScalar sigmaX,
                                         SkScalar sigmaY,
                                         SkTileMode tileMode,
            ...
            .class_function(
                    "MakeDilate",
                    optional_override([](SkScalar radiusX,
                                         SkScalar radiusY,
                                         sk_sp<SkImageFilter> input) -> sk_sp<SkImageFilter> {
            ...
            .class_function(
                    "MakeRuntimeShader",
                    optional_override([](SkRuntimeEffectBuilder& builder,
                                         std::string childShaderName,
                                         sk_sp<SkImageFilter> input) -> sk_sp<SkImageFilter> {
                        return SkImageFilters::RuntimeShader(builder, childShaderName, input);
                    }))
            .class_function(
                    "_MakeDropShadow",
                    optional_override([](SkScalar dx,
                                         SkScalar dy,
                                         SkScalar sigmaX,
                                         SkScalar sigmaY,
                                         WASMPointerF32 cPtr,
    ...
    class_<SkPathEffect>("PathEffect")
            ...
            .class_function("_MakeLine2D",
                            optional_override(
                                    [](SkScalar width, WASMPointerF32 mPtr) -> sk_sp<SkPathEffect> {
                                        SkMatrix matrix;
                                        const SkScalar* nineMatrixValues =
                                                reinterpret_cast<const SkScalar*>(mPtr);
    ...
    class_<SkPath>("Path")
            ...
            .function(
                    "_addCircle",
                    optional_override([](SkPath& self, SkScalar x, SkScalar y, SkScalar r, bool ccw)
                                              -> void {
    ...
    class_<SkShader>("Shader")
            ...
            .class_function("MakeFractalNoise",
                            optional_override([](SkScalar baseFreqX,
                                                 SkScalar baseFreqY,
                                                 int numOctaves,
                                                 SkScalar seed,
                                                 int tileW,
    ...
    class_<SkRuntimeEffectBuilder>("RuntimeEffectBuilder")
            ...
            .function("_getUniform",
                      optional_override([](SkRuntimeEffectBuilder& self,
                                           std::string name,
                                           WASMPointerF32 fPtr) -> bool {
                          auto uniform = self.uniform(name.c_str());
                          if (!uniform.fVar) {
                              return false;
                          }

                          // Copy uniform data to the provided buffer
                          const void* data = static_cast<const uint8_t*>(self.uniforms()->data()) +
                                             uniform.fVar->offset;
                          size_t size = uniform.fVar->sizeInBytes();

                          if (fPtr) {
                              float* outPtr = reinterpret_cast<float*>(fPtr);
                              memcpy(outPtr, data, size);
                              return true;
                          }
                          return false;
                      }))
            .function("_setUniform",
                      optional_override([](SkRuntimeEffectBuilder& self,
                                           ...
                                           size_t fLen) -> bool {
                          if (!fPtr) {
                              // Allow null pointer - just skip the operation
                              return true;
                          }
                          ...
                          size_t expectedFloats = uniform.fVar->sizeInBytes() / sizeof(float);
                          if (fLen != expectedFloats) {
                              return false;  // Size mismatch
                          }

                          const float* floatData = reinterpret_cast<const float*>(fPtr);
                          return uniform.set(floatData, fLen);
                      }))
            .function("setUniformFloat",
                      optional_override([](SkRuntimeEffectBuilder& self,
                          ...
                          return uniform.set(&value, 1);
                      }))
            .function("getChildShader",
                      optional_override([](SkRuntimeEffectBuilder& self,
                                           std::string name) -> sk_sp<SkShader> {
                          ...
                          if (child.fChild->index >= 0 && child.fChild->index < children.size()) {
                              const auto& childPtr = children[child.fChild->index];
                              if (auto shader = childPtr.shader()) {
            ...
            .function("getChildColorFilter",
                      optional_override([](SkRuntimeEffectBuilder& self,
                                           std::string name) -> sk_sp<SkColorFilter> {
                          auto child = self.child(name.c_str());
                          if (!child.fChild) {
                              return nullptr;
                          }
                          ...
                          if (child.fChild->index >= 0 && child.fChild->index < children.size()) {
                              const auto& childPtr = children[child.fChild->index];
                              if (auto colorFilter = childPtr.colorFilter()) {
                                  return sk_ref_sp(colorFilter);
                              }
                          }

                          return nullptr;
                      }))
            .function("getChildBlender",
                      optional_override([](SkRuntimeEffectBuilder& self,
                                           std::string name) -> sk_sp<SkBlender> {
                          auto child = self.child(name.c_str());
                          if (!child.fChild) {
                              return nullptr;
                          }
                          ...
                          if (child.fChild->index >= 0 && child.fChild->index < children.size()) {
                              const auto& childPtr = children[child.fChild->index];
                              if (auto blender = childPtr.blender()) {
                                  return sk_ref_sp(blender);
                              }
                          }
            ...
            .function("setChildShader",
                      optional_override([](SkRuntimeEffectBuilder& self,
                          ...
                          child = shader;  // This handles null shaders correctly
                          return true;
                      }))
            .function("setChildColorFilter",
                      optional_override([](SkRuntimeEffectBuilder& self,
                                           std::string name,
                                           sk_sp<SkColorFilter> colorFilter) -> bool {
                          auto child = self.child(name.c_str());
                          if (!child.fChild) {
                              return false;
                          }

                          child = colorFilter;  // This handles null colorFilters correctly
                          return true;
                      }))
            .function("setChildBlender",
                      optional_override([](SkRuntimeEffectBuilder& self,
                                           std::string name,
                                           sk_sp<SkBlender> blender) -> bool {
                          auto child = self.child(name.c_str());
                          if (!child.fChild) {
                              return false;
                          }

                          child = blender;  // This handles null blenders correctly
                          return true;
                      }))
            .function("_makeShader",
                      optional_override([](SkRuntimeEffectBuilder& self,
                                           WASMPointerF32 mPtr) -> sk_sp<SkShader> {
                          OptionalMatrix localMatrix(mPtr);
                          return self.makeShader(mPtr ? &localMatrix : nullptr);
                      }))
    ...
    class_<SkTextBlob>("TextBlob")
            ...
            .class_function("_MakeFromRSXform",
                            optional_override([](WASMPointerU8 sptr,
                                ...
                                return SkTextBlob::MakeFromRSXform(
                                        str, strBtyes, xforms, font, SkTextEncoding::kUTF8);
                            }),
                            allow_raw_pointers())
            .class_function("_MakeFromRSXformGlyphs",
                            optional_override([](WASMPointerU16 gPtr,
                                ...
                                return SkTextBlob::MakeFromRSXform(
                                        glyphs, byteLen, xforms, font, SkTextEncoding::kGlyphID);
                            }),
                            allow_raw_pointers())
